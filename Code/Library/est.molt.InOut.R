...
surv.molt <- function(data, y){
  Ap_Sept_Data <- subset(data, Date == "2014-04-07" | Date == "2014-09-17"| Date == "2020-04-03" |Date == "2020-09-01" |Date == "2020-09-15", Date : CPUE)
  
  #create pre and post df for each size class 
  Pre_molt_juv <- subset(Ap_Sept_Data, Carapace.Width <= 139 & Month == "April" & Year == y) #juveniles moving into sub-adults
  
  # sub-adults have to be spilt into two b/c of zhang LR calc
  Pre_molt_sub1 <- subset(Ap_Sept_Data, Carapace.Width %in% c(140:149) & Month == "April"& Year == y)
  Pre_molt_sub2 <- subset(Ap_Sept_Data, Carapace.Width %in% c(150:159) & Month == "April"& Year == y)
  
  Post_molt_sub <- subset(Ap_Sept_Data, Carapace.Width %in% c(140:159) & Month == "September" & Year == y)
