...
# Account for pots that were sampled but no sub-legal males were present in that pot
  ...
  df$Pot <- factor(df$Pot, levels=1:max(total.pots)) #make factore so cast across max pots (12)
  Resample_CPUE <-cast(df, Date~Pot, add.missing=TRUE, fill=0) #reformat the df to include Pots not present in wide format

# For every pot greater than the actual total number of pots per sample date, make NA    
  for(d in 1:(nrow(Resample_CPUE)-1)){
    Resample_CPUE[d, (total.pots[d]+2) : (length(Resample_CPUE))] <- NA 
  }
